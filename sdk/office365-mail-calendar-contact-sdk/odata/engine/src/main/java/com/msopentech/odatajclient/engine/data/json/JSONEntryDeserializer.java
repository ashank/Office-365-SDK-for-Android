...
public class JSONEntryDeserializer
        extends ODataJsonDeserializer<AbstractJSONEntry<AbstractLinkResource<? extends EntryResource, ? extends FeedResource>>> {

    private String getTitle(final Map.Entry<String, JsonNode> entry) {
        return entry.getKey().substring(0, entry.getKey().indexOf('@'));
    }

    private String setV3Inline(final String name, final String suffix, final ObjectNode tree,
            final ObjectCodec codec, final JSONV3Link link) throws IOException {
        ...
        if (tree.has(entryNamePrefix)) {
            ...
            if (inline instanceof ArrayNode) {
                final JSONV3Feed feed = client.getResourceFactory().newFeed(JSONV3Feed.class);
                final Iterator<JsonNode> entries = ((ArrayNode) inline).elements();
                while (entries.hasNext()) {
                    final JsonParser inlineParser = entries.next().traverse();
                    inlineParser.setCodec(codec);
                    feed.addEntry((JSONV3Entry) inlineParser.readValuesAs(client.getResourceFactory().entryClassForFeed(feed.getClass())));
                }
    ...
    private String setV4Inline(final String name, final String suffix, final ObjectNode tree,
    ...
    @Override
    protected AbstractJSONEntry<AbstractLinkResource<?, ?>> doDeserializeV3(JsonParser parser,
        ...
        AbstractJSONEntry<AbstractLinkResource<?, ?>> entry;
        try {
            entry = (AbstractJSONEntry<AbstractLinkResource<?, ?>>)client.getResourceFactory().entryClassForFormat(ODataPubFormat.JSON).newInstance();
        } catch (Exception e) {
        ...
        if (tree.hasNonNull(ODataConstants.JSON_READ_LINK)) {
            final JSONV3Link link = new JSONV3Link();
        ...
        while (itor.hasNext()) {
            ...
            if (field.getKey().endsWith(ODataConstants.JSON_NAVIGATION_LINK_SUFFIX) || field.getKey().endsWith(ODataConstants.JSON_NAVIGATION_LINK_ODATA_4_SUFFIX)) {
                ...
                toRemove.add(setV3Inline(field.getKey(),
        ...
        try {
            ...
            DOMTreeUtilsV3.buildSubtree(properties, tree);
    ...
    @Override
    protected AbstractJSONEntry<AbstractLinkResource<?, ?>> doDeserializeV4(JsonParser parser,
            DeserializationContext ctxt) throws IOException, JsonProcessingException {
        final ObjectNode tree = (ObjectNode) parser.getCodec().readTree(parser);
        ...
        while (itor.hasNext()) {
            ...
            if (field.getKey().endsWith(ODataConstants.JSON_NAVIGATION_LINK_SUFFIX) || field.getKey().endsWith(ODataConstants.JSON_NAVIGATION_LINK_ODATA_4_SUFFIX)) {
                ...
                toRemove.add(field.getKey());
                toRemove.add(setV4Inline(field.getKey(),
                        field.getKey().endsWith(ODataConstants.JSON_NAVIGATION_LINK_SUFFIX) ?
                        ODataConstants.JSON_NAVIGATION_LINK_SUFFIX :
                        ODataConstants.JSON_NAVIGATION_LINK_ODATA_4_SUFFIX, tree, parser.getCodec(), link));
            } else if (field.getKey().endsWith(ODataConstants.JSON_ASSOCIATION_LINK_SUFFIX)) {
        ...
        try {
            ...
            DOMTreeUtilsV4.buildSubtree(properties, tree);
